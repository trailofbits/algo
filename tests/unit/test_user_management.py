...
def test_user_config_generation():
    ...
    for user in users:
        ...
        assert user in wg_path, f"Username not in WireGuard config path"
        
        # Test IPsec config path  
        ipsec_path = f"configs/{server_name}/ipsec/{user}.p12"
        assert user in ipsec_path, f"Username not in IPsec config path"
        
        # Test SSH tunnel config path
        ssh_path = f"configs/{server_name}/ssh-tunnel/{user}.pem"
        assert user in ssh_path, f"Username not in SSH config path"
